...
def call_fx_postgres(conn, sp_name, sp_values, s_percent):
	frow = ''
	cur = conn.cursor()
...
def call_fx_postgres_many_rows(conn, sp_name, sp_values, s_percent):
	frow = ''
	cur = conn.cursor()
	build_call = "SELECT * FROM " + sp_name + " " + s_percent
	cur.execute(build_call, (sp_values))
	# # process the result set
	row = cur.fetchall()
	# close the communication with the PostgreSQL database server
...
def get_feat_list(feat_json, sn):
	feat_list = ()
	for m in feat_json:
		if m != 'properties':
			for s in feat_json[m]['shapefiles']:
				feat_shp_name = s['file_feat_name']
				if feat_shp_name == sn:
					fieldmap = s['ogr_fieldmap']
					feat_type_id = s['geo_object_feature_type']
					geom_type_id = s['feat_geom_type']
					url_id = s['source_url']
					if len(fieldmap) > 0:
						feat_list = (feat_shp_name, fieldmap, feat_type_id, geom_type_id, url_id)
	return feat_list